    ...
    navigator.mediaDevices.getUserMedia(constraints)
      ...
      .then(() => {
        ...
        serverConnection.onopen = event => {
          serverConnection.send(JSON.stringify({ 'displayName': localDisplayName, 'uuid': localUuid, 'dest': 'all', 'isInitiator': isInitiator }));
        }
      }).catch(errorHandler);
...
function gotMessageFromServer(message) {
  ...
  if (signal.isInitiator)
    isInitiator = signal.isInitiator;

  // Ignore messages that are not for us or from ourselves
  if (peerUuid == localUuid || (signal.dest != localUuid && signal.dest != 'all')) return;

  if (signal.displayName && signal.dest == 'all' && isInitiator) {
